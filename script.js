(function(){
  ...
  function renderSizes(){
    ...
    SIZES.forEach(s => {
      ...
      btn.addEventListener('click', () => {
        ...
        renderSizes();
        updateSummary();
        document.getElementById('sizeDisplay').value = s;
      });
  ...
  function updateSummary(){
    ...
    document.getElementById('colorDisplay').value = c.name;
  }
  ...
  function initForm(){
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (city.length < 2){ showError('city','Merci d\'indiquer votre ville.'); valid = false; }
      if (address.length < 5){ showError('address','Merci de détailler votre adresse.'); valid = false; }
      if (!state.size){
